...
// ---------------------------------------------------
// Variables for new visualization
var grid = null;
...
// a list of object IDs from MATRXS, mapping to the corresponding ID's of the html object
var matrxs_objects = [];
// the list with objects from the previous iteration
var matrxs_objects_prev = [];
...
function fixCanvasSize() {
    ...
    // get grid element from html
    grid = document.getElementById("grid");

    // resize to current window size
    grid_width = document.body.clientWidth; //document.width is obsolete
    grid_height = document.body.clientHeight; //document.height is obsolete
 ...
 */
function fixTileSize(grid_width, grid_height) {

    // calc the pixel per cell ratio in the x and y direcetion
    var px_per_cell_x = Math.round(grid_width / rows);
    var px_per_cell_y = Math.round(grid_height / columns);
...
function updateGridSize(grid_size) {
    if (grid_size[0] != rows || grid_size[1] != columns) {
        ...
        fixTileSize(grid_width, grid_height);
    }
}
...
/**
 * Draw a background with the provided image, colour, or otherwise the default colour
 */
function drawBg() {
    // clear any background image
    grid.setAttribute("background-image", "");

    // draw the background image if specified
    if (bgImage != null) {
        // set the background image of the grid element
        grid.setAttribute("background-image", img.src);

    // otherwise, colour the background image
    } else {
        grid.setAttribute("background-color", bgTileColour);
    }
}
...
function draw(new_tick) {
    ...
    if (firstDraw) {
        ...
        console.log("First draw, resetting grid and tile sizes");
        fixCanvasSize();
        firstDraw = false;
    ...
    if (new_tick) {
        ...
        matrxs_objects_prev = matrxs_objects;
        matrxs_objects = [];
